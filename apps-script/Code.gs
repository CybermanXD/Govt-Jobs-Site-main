...
function classCell_(rowHtml, className) { const m=String(rowHtml).match(new RegExp('<(?:td|th)\\b[^>]*class=["\'][^"\']*\\b'+className+'\\b[^"\']*["\'][^>]*>([\\s\\S]*?)<\\/(?:td|th)>','i')); return m?m[1]:''; }
function titleFromCell_(cellHtml) { const desktop=String(cellHtml).match(/<span\b[^>]*class=["'][^"']*kar-t-desk[^"']*["'][^>]*>([\s\S]*?)<\/span>/i); return text_(desktop?desktop[1]:cellHtml); }
function boardFromTitle_(title) { const clean=text_(title).replace(/\s+(?:Vacancy|Recruitment|Jobs?)\s+20\d{2}[\s\S]*$/i,'').trim(); return clean||text_(title).split(/\s+-\s+|\s+/)[0]||''; }
function parseTablePage_(html, source) {
  const jobs=[], rows=String(html).match(/<tr\b[\s\S]*?<\/tr>/gi)||[];
  rows.forEach(row=>{
    const modernTitleCell=classCell_(row,'kar-c-title');
    if(modernTitleCell){
      const title=titleFromCell_(modernTitleCell), href=hrefMatching_(modernTitleCell,title)||hrefMatching_(modernTitleCell,'Vacancy')||hrefMatching_(row,'View');
      if(!title||!href)return;
      const board=boardFromTitle_(title), qualification=text_(classCell_(row,'kar-c-qual')), lastDate=dateIso_(classCell_(row,'kar-c-last')), vacancy=text_(classCell_(row,'kar-c-vac')), sourceName='FreeJobAlert '+source.category, count=(vacancy.match(/\d[\d,]*/)||[])[0];
      jobs.push(normalizeJob_({title:title,board:board,qualification:qualification,lastDate:lastDate,source:sourceName,url:canonicalUrl_(href,source.url),state:source.forcedState||inferState_([title,board,sourceName].join(' '))||null,postCount:count?Number(count.replace(/,/g,'')):null,location:null}));
      return;
    }
    const href=hrefMatching_(row,'Get Details'); if(!href)return;
    const c=cells_(row); if(c.length<6)return;
    const title=c[2],board=c[1],sourceName='FreeJobAlert '+source.category,count=(title.match(/(\d[\d,]*)\s*Posts?/i)||[])[1];
    jobs.push(normalizeJob_({title:title,board:board,qualification:c[3],lastDate:dateIso_(c[5]),source:sourceName,url:canonicalUrl_(href,source.url),state:source.forcedState||inferState_([title,board,sourceName].join(' '))||null,postCount:count?Number(count.replace(/,/g,'')):null,location:null}));
  });
  return jobs;
}
...
function parseSearchPage_(html, source) { const jobs=[]; const blocks=String(html).split(/<div\b[^>]*class=["'][^"']*org_tab[^"']*["'][^>]*>/i).slice(1); blocks.forEach(block=>{ block=block.split(/<div\b[^>]*class=/i)[0]; const fields=fieldMap_(block); const board=text_((block.match(/<span\b[^>]*>([\s\S]*?)<\/span>/i)||[])[1]||''); const post=fields['Post Name']||fields['Post Name ']||''; const vacancy=fields['No. of Vacancy']||fields['No. of Vacancies']||fields['Vacancies']||''; const location=fields['Location']||''; const title=board&&post&&post.indexOf(board)<0?board+' '+post:(post||board), sourceName='FreeJobAlert '+source.category; const count=(vacancy.match(/\d[\d,]*/)||[])[0]; const href=hrefMatching_(block,'Apply Now'); jobs.push(normalizeJob_({title:title,board:board,qualification:fields['Qualification']||fields['Qualifications']||'',lastDate:dateIso_(fields['Last Date to Apply']||fields['Last Date']||''),source:sourceName,url:canonicalUrl_(href,source.url),state:source.forcedState||inferState_([title,board,sourceName,location].join(' '))||null,postCount:count?Number(count.replace(/,/g,'')):null,location:location||null})); }); return jobs.filter(x=>x.title&&x.url); }
...
function inferState_(text) {
  const groups=[
    ['Andhra Pradesh',['andhra pradesh','andhra','amaravati','visakhapatnam','vijayawada','tirupati','korukonda','appsc','apsrtc']],['Arunachal Pradesh',['arunachal pradesh','itanagar','arunachal psc','appsc arunachal']],['Assam',['assam','guwahati','dispur','jorhat','dibrugarh','apsc','assam psc']],['Bihar',['bihar','patna','bpsc','bihar psc','bssc']],['Chhattisgarh',['chhattisgarh','chattisgarh','raipur','bilaspur','cgpsc','cgvyapam']],['Goa',['goa','panaji','goa psc','goa ssc','iit goa']],['Gujarat',['gujarat','ahmedabad','gandhinagar','surat','gpsc','gsssb']],['Haryana',['haryana','gurugram','gurgaon','faridabad','panchkula','hpsc','hssc']],['Himachal Pradesh',['himachal pradesh','shimla','dharamshala','hppsc','hpsssb']],['Jharkhand',['jharkhand','ranchi','jamshedpur','jpsc','jssc']],['Karnataka',['karnataka','bengaluru','bangalore','mysuru','mysore','dharwad','kpsc','kea']],['Kerala',['kerala','thiruvananthapuram','trivandrum','kochi','cochin','kozhikode','kerala psc']],['Madhya Pradesh',['madhya pradesh','bhopal','indore','jabalpur','mppsc','mpesb','mppeb']],['Maharashtra',['maharashtra','mumbai','pune','nagpur','nashik','mpsc','mahatransco','mahadiscom']],['Manipur',['manipur','imphal','manipur psc','mpsc manipur']],['Meghalaya',['meghalaya','shillong','meghalaya psc']],['Mizoram',['mizoram','aizawl','mizoram psc','phed mizoram']],['Nagaland',['nagaland','kohima','dimapur','nagaland psc']],['Odisha',['odisha','orissa','bhubaneswar','bhubaneshwar','cuttack','opsc','osssc','ossc']],['Punjab',['punjab','chandigarh punjab','ludhiana','amritsar','patiala','ppsc','psssb']],['Rajasthan',['rajasthan','jaipur','jodhpur','udaipur','rpsc','rsmssb']],['Sikkim',['sikkim','gangtok','sikkim psc']],['Tamil Nadu',['tamil nadu','chennai','coimbatore','madurai','tnpsc','tnusrb']],['Telangana',['telangana','hyderabad','secunderabad','tspsc','tgpsc']],['Tripura',['tripura','agartala','tpsc','tripura psc']],['Uttar Pradesh',['uttar pradesh','lucknow','prayagraj','allahabad','kanpur','varanasi','noida','uppsc','upsssc','up police']],['Uttarakhand',['uttarakhand','uttaranchal','dehradun','haridwar','ukpsc','uksssc']],['West Bengal',['west bengal','kolkata','calcutta','wbpsc','wbssc']],['Andaman and Nicobar Islands',['andaman and nicobar','port blair']],['Chandigarh',['chandigarh','chandigarh administration']],['Dadra and Nagar Haveli and Daman and Diu',['dadra and nagar haveli','daman and diu','daman','silvassa']],['Delhi',['delhi','new delhi','ncr','dsssb','delhi police']],['Jammu and Kashmir',['jammu and kashmir','jammu kashmir','jammu','srinagar','jkpsc','jkssb']],['Ladakh',['ladakh','leh','kargil']],['Lakshadweep',['lakshadweep','kavaratti']],['Puducherry',['puducherry','pondicherry']]
  ];
  const normalized=' '+String(text||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim()+' ';
  for(let i=0;i<groups.length;i++)for(let j=0;j<groups[i][1].length;j++){const term=groups[i][1][j].replace(/[^a-z0-9]+/g,' ').trim();if(normalized.indexOf(' '+term+' ')>=0)return groups[i][0];}
  return '';
}
