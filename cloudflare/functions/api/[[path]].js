...
export async function onRequestGet(context) {
  ...
  try {
    ...
    return new Response(body, {status: response.ok ? 200 : 502, headers: {'content-type': 'application/json; charset=utf-8','cache-control': `public, max-age=${cacheSeconds}`,'access-control-allow-origin': '*'}});
  } catch (error) {
    return Response.json({ok: false, error: 'Backend unavailable', detail: error.message}, {status: 502});
  }
}
