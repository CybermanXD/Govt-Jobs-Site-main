...
function desiredImportantLink(link) { return link&&['applyOnline','officialNotification','officialWebsite'].includes(link.type)&&validHttpUrl(link.url); }
...
function mergeJobDetails(job, detail) { const data=detail&&typeof detail==='object'?detail:{};return {...data,companyName:data.companyName||job.board||'',postName:data.postName||job.title||'',noOfPosts:data.noOfPosts||job.postCount||'',qualification:data.qualification||job.qualification||'',location:data.location||job.location||'',lastDate:data.lastDate||job.lastDate||'',url:data.url||job.url||''}; }
function detailsLoadingMarkup() {
  const lines = widths => widths.map(width=>`<span class="details-skeleton-line" style="--skeleton-width:${width}%"></span>`).join('');
  return `<div class="details-loader" role="status" aria-live="polite"><p class="details-loader-title">Loading verified job details<span class="details-loader-dots" aria-hidden="true"><span></span><span></span><span></span></span></p><div class="details-skeleton" aria-hidden="true"><div class="details-skeleton-group">${lines([38,100,84])}</div><div class="details-skeleton-group">${lines([46,94,72])}</div><div class="details-skeleton-group">${lines([32,88])}</div></div></div>`;
}
async function openModal(id) {
  const job=jobs.find(j=>j.id===id); if(!job)return;
  ['title','board','qualification','location','postname','postcount','salary','agelimit','company','advt','source'].forEach(key=>{const el=$(`modal-${key}`);if(el)el.textContent='N/A';});
  ...
  $('modal-details').innerHTML=detailsLoadingMarkup();['modal-important-dates','modal-selection-process','modal-general-instructions','modal-how-to-apply','modal-important-links'].forEach(x=>$(x).innerHTML='');
  ...
  try { const response=detailsByUrl[job.url]||await api(`job_details?id=${encodeURIComponent(job.id)}`);if(response&&response.error)throw new Error(response.error);const data=mergeJobDetails(job,response);detailsByUrl[job.url]=response;
    ...
    const links=(Array.isArray(data.importantLinks)?data.importantLinks:[]).filter(desiredImportantLink);$('modal-important-links').innerHTML=links.length?`<h4>Important Links</h4><ul>${links.map(x=>`<li>${esc(x.label||'')} <a target="_blank" rel="noopener noreferrer" href="${esc(x.url)}">${esc(x.display||x.url)}</a></li>`).join('')}</ul>`:`<p>${esc(data.officialNotificationStatus||'')}</p>`;
